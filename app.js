  ...
  function mapData(data) {
    return data.map((row) => {
      ...
      if (typeof totalCharged === "string") {
        totalCharged = totalCharged.replace(/\s[A-Z]{3}/, "").replace(".", ",");
      }
...
//    try {
//       const response = await fetch("C:\Users\HP\Desktop\MMT_Projects\Currency_Change\factors.json");
//       if (!response.ok) {
...
//       mappedData = addShipmentColumn(mappedData, factors); // Add the Factor column
//       createTable(mappedData);
//    }
...
//    function addShipmentColumn(mappedData, factors) {
//       return mappedData.map((row) => {
//          const country = row.Country;
